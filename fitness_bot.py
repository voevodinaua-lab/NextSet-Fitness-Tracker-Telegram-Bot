...
# Состояния бота - ИСПРАВЛЕНО: теперь 14 состояний и range(14)
(
...
) = range(14)  # ИСПРАВЛЕНО: было 13, стало 14
...
def update_statistics(user_id, training):
    ...
    for exercise in training['exercises']:
        if exercise.get('is_cardio'):  # Используем get для безопасного доступа
            cardio_count += 1
        else:
            strength_count += 1
...
async def show_measurements_history(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Показать историю замеров - ИСПРАВЛЕНО"""
    user_id = update.message.from_user.id
    user_data = get_user_data(user_id)
    ...
    # ИСПРАВЛЕНО: правильный подсчет индекса
...
async def show_training_history(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Показать историю тренировок - ИСПРАВЛЕНО"""
    user_id = update.message.from_user.id
    user_data = get_user_data(user_id)
    ...
    # ИСПРАВЛЕНО: правильный подсчет номера тренировки
