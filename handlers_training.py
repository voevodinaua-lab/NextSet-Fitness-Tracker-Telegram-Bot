...
async def handle_training_menu_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"🔍 Сравнение с '💪 Силовые упражнения': '{text}' == '💪 Силовые упражнения' = {text == '💪 Силовые упражнения'}")
    print(f"🔍 Сравнение с '🏃 Кардио': '{text}' == '🏃 Кардио' = {text == '🏃 Кардио'}")
    print(f"🔍 Сравнение с '✏️ Добавить свое упражнение': '{text}' == '✏️ Добавить свое упражнение' = {text == '✏️ Добавить свое упражнение'}")
    print(f"🔍 Сравнение с '🏁 Завершить тренировку': '{text}' == '🏁 Завершить тренировку' = {text == '🏁 Завершить тренировку'}")
    
    # Проверка по частям
    print(f"🔍 Содержит 'Силовые': {'Силовые' in text}")
    print(f"🔍 Содержит 'Кардио': {'Кардио' in text}")
    print(f"🔍 Содержит 'Добавить': {'Добавить' in text}")
    print(f"🔍 Содержит 'Завершить': {'Завершить' in text}")
    ...
    if text == '💪 Силовые упражнения':
        ...
        print(f"⚠️ DEBUG: Переход в fallback!")
...
async def handle_training_menu_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"🔍 Fallback: коды символов: {[ord(c) for c in text]}")
    
    # Временно показываем простой текст без кнопок
    await update.message.reply_text(
        f"DEBUG: Fallback сработал для текста: '{text}'\n"
        f"Длина: {len(text)}, Коды: {[ord(c) for c in text]}"
    )
    return TRAINING_MENU
